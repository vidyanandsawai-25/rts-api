...
/// <summary>
/// Comprehensive tests for AssetFloorFactorCVController covering GetAll, GetById, Create, Update,
/// Delete and Bulk (Create/Update/Delete) endpoints, including floor/year-range existence
/// validation and duplicate-combination conflicts - aligned with the AssetNatureFactorCVController
/// shape (Bulk endpoints instead of Purge/hard-delete, since both entities equally support
/// IHardDeletable/BaseCommonCrudService but only one surfaced Bulk endpoints and the other Purge).
/// </summary>
public class AssetFloorFactorCVControllerTests
{
    private static AssetFloorFactorCVController Create(out Mock<IAssetFloorFactorCVService> service)
    {
        service = new Mock<IAssetFloorFactorCVService>();
        var logger = new Mock<ILogger<AssetFloorFactorCVController>>();
        return new AssetFloorFactorCVController(service.Object, logger.Object);
    }
    ...
    {
        var attribute = typeof(AssetFloorFactorCVController)
            .GetCustomAttributes(typeof(RouteAttribute), false)
            .FirstOrDefault() as RouteAttribute;

        Assert.NotNull(attribute);
        Assert.Equal("api/asset-management/floor-factor-cv", attribute!.Template);
    }
    ...
    public async Task GetAll_WithValidQuery_ReturnsOkWithPagedResult()
    {
        ...
        var query = new AssetFloorFactorCVQueryParameters { PageNumber = 1, PageSize = 10 };
        var data = new List<AssetFloorFactorCVDto>
        {
            new() { Id = 1, FloorId = 10, FloorDescription = "Ground Floor", YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m, IsActive = true },
            new() { Id = 2, FloorId = 20, FloorDescription = "First Floor", YearRangeCVId = 5, FactorWithLift = 1.1m, FactorWithoutLift = 0.9m, IsActive = true }
        };
        var pagedResult = new PagedResult<AssetFloorFactorCVDto>(data, 2, 1, 10);
        ...
        var returnedData = Assert.IsType<PagedResult<AssetFloorFactorCVDto>>(okResult.Value);
        Assert.Equal(2, returnedData.TotalCount);
        Assert.Equal(2, returnedData.Items.Count());
        Assert.Contains(returnedData.Items, x => x.Id == 1 && x.FloorDescription == "Ground Floor");
        Assert.Contains(returnedData.Items, x => x.Id == 2 && x.FloorDescription == "First Floor");
    }
    ...
    {
        var controller = Create(out var service);
        var query = new AssetFloorFactorCVQueryParameters();
        service.Setup(s => s.GetAllAsync(query, It.IsAny<CancellationToken>()))
    ...
    {
        var controller = Create(out var service);
        var query = new AssetFloorFactorCVQueryParameters { FloorId = 10, YearRangeCVId = 5, IsActive = true };
        var pagedResult = new PagedResult<AssetFloorFactorCVDto>(
            new List<AssetFloorFactorCVDto> { new() { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m, IsActive = true } },
            1, 1, 10);
    ...
    {
        var controller = Create(out var service);
        var dto = new AssetFloorFactorCVDto { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m, IsActive = true };
        service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(dto);

        var result = await controller.GetById(1, CancellationToken.None);

        var okResult = Assert.IsType<OkObjectResult>(result);
        var returnedDto = Assert.IsType<AssetFloorFactorCVDto>(okResult.Value);
        Assert.Equal(1, returnedDto.Id);
        Assert.Equal(1.2m, returnedDto.FactorWithLift);
    }
    ...
    {
        var controller = Create(out var service);
        service.Setup(s => s.GetByIdAsync(999, It.IsAny<CancellationToken>())).ReturnsAsync((AssetFloorFactorCVDto?)null);
    ...
    public async Task GetById_WithCancellationToken_PassesTokenToService()
    {
        var controller = Create(out var service);
        var cancellationToken = new CancellationToken();
        service.Setup(s => s.GetByIdAsync(1, cancellationToken)).ReturnsAsync((AssetFloorFactorCVDto?)null);
    ...
    {
        var controller = Create(out var service);
        var createDto = new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m, IsActive = true, CreatedBy = 1 };
        var createdDto = new AssetFloorFactorCVDto { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m, IsActive = true };
        ...
        var okResult = Assert.IsType<OkObjectResult>(result);
        var response = Assert.IsType<ApiResponse<AssetFloorFactorCVDto>>(okResult.Value);
    ...
    {
        var controller = Create(out var service);
        var createDto = new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m };
        var cancellationToken = new CancellationToken();
        service.Setup(s => s.CreateAsync(createDto, cancellationToken))
            .ReturnsAsync(new AssetFloorFactorCVDto { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m });
    ...
    [Fact]
    public async Task Create_WithNonExistentFloorId_ThrowsValidationException()
    {
        // Business rule enforced by AssetFloorFactorCVService.ValidateForCreateAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetFloorFactorCVDto { FloorId = 999, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m };

        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.FloorId), "AssetFloorFactorCV_FloorId_Invalid", OperationType.Create));
    ...
    public async Task Create_WithNonExistentYearRangeCVId_ThrowsValidationException()
    {
        // Business rule enforced by AssetFloorFactorCVService.ValidateForCreateAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 999, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m };

        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.YearRangeCVId), "AssetFloorFactorCV_YearRangeCVId_Invalid", OperationType.Create));

        await Assert.ThrowsAsync<ValidationException>(() => controller.Create(createDto, CancellationToken.None));
    }

    [Fact]
    public async Task Create_WithDuplicateFloorAndYearRangeCombination_ThrowsValidationException()
    {
        // Business rule enforced by AssetFloorFactorCVService.ValidateForCreateAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m };
        ...
        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.FloorId), "AssetFloorFactorCV_Duplicate", OperationType.Create));
    ...
    public async Task Create_WhenNonValidationDuplicateConstraintErrorOccurs_ReturnsConflict()
    {
        ...
        var response = Assert.IsType<ApiResponse<AssetFloorFactorCVDto>>(conflictResult.Value);
    ...
    {
        var controller = Create(out var service);
        var updateDto = new UpdateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.5m, FactorWithoutLift = 1.2m, IsActive = true, UpdatedBy = 1 };
        var updatedDto = new AssetFloorFactorCVDto { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.5m, FactorWithoutLift = 1.2m, IsActive = true };
        ...
        var response = Assert.IsType<ApiResponse<AssetFloorFactorCVDto>>(okResult.Value);
        Assert.True(response.Success);
        Assert.Equal(1.5m, response.Items!.FactorWithLift);
    }
    ...
    {
        var controller = Create(out var service);
        var updateDto = new UpdateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m };
        service.Setup(s => s.UpdateAsync(999, updateDto, It.IsAny<CancellationToken>())).ReturnsAsync((AssetFloorFactorCVDto?)null);
    ...
    [Fact]
    public async Task Update_WithNonExistentFloorId_ThrowsValidationException()
    {
        var controller = Create(out var service);
        var updateDto = new UpdateAssetFloorFactorCVDto { FloorId = 999, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m };

        service.Setup(s => s.UpdateAsync(1, updateDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(updateDto.FloorId), "AssetFloorFactorCV_FloorId_Invalid", OperationType.Update));
    ...
    {
        // Business rule enforced by AssetFloorFactorCVService.ValidateForDeactivationAsync
        var controller = Create(out var service);
        var updateDto = new UpdateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m, IsActive = false };
    ...
    {
        // Business rule enforced by AssetFloorFactorCVService.ValidateForDeleteAsync
        var controller = Create(out var service);
    ...
    public async Task BulkCreate_WithValidItems_ReturnsOkWithSuccessCount()
    {
        ...
        var items = new[]
        {
            new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m },
            new CreateAssetFloorFactorCVDto { FloorId = 20, YearRangeCVId = 5, FactorWithLift = 1.1m, FactorWithoutLift = 0.9m }
        };
        var createdItems = new List<AssetFloorFactorCVDto>
        {
            new() { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m },
            new() { Id = 2, FloorId = 20, YearRangeCVId = 5, FactorWithLift = 1.1m, FactorWithoutLift = 0.9m }
        };
        var bulkResult = new BulkResult<AssetFloorFactorCVDto>(2, 0, createdItems);
        ...
        var okResult = Assert.IsType<OkObjectResult>(result);
        var response = Assert.IsType<ApiResponse<BulkResult<AssetFloorFactorCVDto>>>(okResult.Value);
        Assert.True(response.Success);
    ...
    public async Task BulkCreate_WithNullArray_ReturnsBadRequest()
    {
        ...
        var response = Assert.IsType<ApiResponse<BulkResult<AssetFloorFactorCVDto>>>(badRequest.Value);
        Assert.False(response.Success);
    }
    ...
    {
        var controller = Create(out _);
        var items = Array.Empty<CreateAssetFloorFactorCVDto>();
    ...
    public async Task BulkCreate_WithPartialFailures_ReturnsOkWithMixedResults()
    {
        ...
        {
            new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m },
            new CreateAssetFloorFactorCVDto { FloorId = 999, YearRangeCVId = 5, FactorWithLift = 1.1m, FactorWithoutLift = 0.9m }
        };
        var createdItems = new List<AssetFloorFactorCVDto> { new() { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m } };
        var bulkResult = new BulkResult<AssetFloorFactorCVDto>(1, 1, createdItems, new List<string> { "Item 1: FloorId not found" });
    ...
    {
        var controller = Create(out var service);
        var items = new[] { new CreateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.2m, FactorWithoutLift = 1.0m } };
        ...
        var conflictResult = Assert.IsType<ConflictObjectResult>(result);
        var response = Assert.IsType<ApiResponse<BulkResult<AssetFloorFactorCVDto>>>(conflictResult.Value);
        Assert.False(response.Success);
    }
    ...
    public async Task BulkUpdate_WithValidItems_ReturnsOkWithSuccessCount()
    {
        ...
        var items = new[]
        {
            new BulkUpdateItem<int, UpdateAssetFloorFactorCVDto>(1, new UpdateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.5m, FactorWithoutLift = 1.2m }),
            new BulkUpdateItem<int, UpdateAssetFloorFactorCVDto>(2, new UpdateAssetFloorFactorCVDto { FloorId = 20, YearRangeCVId = 5, FactorWithLift = 1.4m, FactorWithoutLift = 1.1m })
        };
        var updatedItems = new List<AssetFloorFactorCVDto>
        {
            new() { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.5m, FactorWithoutLift = 1.2m },
            new() { Id = 2, FloorId = 20, YearRangeCVId = 5, FactorWithLift = 1.4m, FactorWithoutLift = 1.1m }
        };
        var bulkResult = new BulkResult<AssetFloorFactorCVDto>(2, 0, updatedItems);
    ...
    public async Task BulkUpdate_WithNonExistingIds_ReturnsPartialFailure()
    {
        ...
        {
            new BulkUpdateItem<int, UpdateAssetFloorFactorCVDto>(1, new UpdateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.5m, FactorWithoutLift = 1.2m }),
            new BulkUpdateItem<int, UpdateAssetFloorFactorCVDto>(999, new UpdateAssetFloorFactorCVDto { FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.4m, FactorWithoutLift = 1.1m })
        };
        var updatedItems = new List<AssetFloorFactorCVDto> { new() { Id = 1, FloorId = 10, YearRangeCVId = 5, FactorWithLift = 1.5m, FactorWithoutLift = 1.2m } };
        var bulkResult = new BulkResult<AssetFloorFactorCVDto>(1, 1, updatedItems, new List<string> { "Record with Id '999' not found." });

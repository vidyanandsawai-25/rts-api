...
/// <summary>
/// Comprehensive tests for AssetUseFactorCVController covering GetAll, GetById, Create, Update,
/// Delete and Bulk (Create/Update/Delete) endpoints, including type-of-use/sub-type-of-use/year-range
/// existence validation and duplicate-combination conflicts.
/// </summary>
public class AssetUseFactorCVControllerTests
{
    private static AssetUseFactorCVController Create(out Mock<IAssetUseFactorCVService> service)
    {
        service = new Mock<IAssetUseFactorCVService>();
        var logger = new Mock<ILogger<AssetUseFactorCVController>>();
        return new AssetUseFactorCVController(service.Object, logger.Object);
    }
    ...
    {
        var attribute = typeof(AssetUseFactorCVController)
            .GetCustomAttributes(typeof(RouteAttribute), false)
            .FirstOrDefault() as RouteAttribute;

        Assert.NotNull(attribute);
        Assert.Equal("api/asset-management/use-factor-cv", attribute!.Template);
    }
    ...
    {
        var controller = Create(out var service);
        var query = new AssetUseFactorCVMasterQueryParameters();
        service.Setup(s => s.GetAllAsync(query, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PagedResult<AssetUseFactorCVMasterDto>(new List<AssetUseFactorCVMasterDto>(), 0, 1, 10));
    ...
    {
        var controller = Create(out var service);
        var query = new AssetUseFactorCVMasterQueryParameters { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, IsActive = true };
        var pagedResult = new PagedResult<AssetUseFactorCVMasterDto>(
            new List<AssetUseFactorCVMasterDto> { new() { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m, IsActive = true } },
            1, 1, 10);
    ...
    {
        var controller = Create(out var service);
        var dto = new AssetUseFactorCVMasterDto { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m, IsActive = true };
        service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(dto);

        var result = await controller.GetById(1, CancellationToken.None);

        var okResult = Assert.IsType<OkObjectResult>(result);
        var returnedDto = Assert.IsType<AssetUseFactorCVMasterDto>(okResult.Value);
    ...
    {
        var controller = Create(out var service);
        service.Setup(s => s.GetByIdAsync(999, It.IsAny<CancellationToken>())).ReturnsAsync((AssetUseFactorCVMasterDto?)null);
    ...
    public async Task GetById_WithCancellationToken_PassesTokenToService()
    {
        var controller = Create(out var service);
        var cancellationToken = new CancellationToken();
        service.Setup(s => s.GetByIdAsync(1, cancellationToken)).ReturnsAsync((AssetUseFactorCVMasterDto?)null);
    ...
    {
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m, IsActive = true, CreatedBy = 1 };
        var createdDto = new AssetUseFactorCVMasterDto { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m, IsActive = true };
        ...
        var okResult = Assert.IsType<OkObjectResult>(result);
        var response = Assert.IsType<ApiResponse<AssetUseFactorCVMasterDto>>(okResult.Value);
    ...
    {
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m };
        var cancellationToken = new CancellationToken();
        service.Setup(s => s.CreateAsync(createDto, cancellationToken))
            .ReturnsAsync(new AssetUseFactorCVMasterDto { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m });
    ...
    [Fact]
    public async Task Create_WithNonExistentTypeOfUseId_ThrowsValidationException()
    {
        // Business rule enforced by AssetUseFactorCVService.EnsureTypeOfUseExistsAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 999, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m };

        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.TypeOfUseId), "Type of use with ID 999 not found.", OperationType.Create));

        await Assert.ThrowsAsync<ValidationException>(() => controller.Create(createDto, CancellationToken.None));
    }

    [Fact]
    public async Task Create_WithNonExistentSubTypeOfUseId_ThrowsValidationException()
    {
        // Business rule enforced by AssetUseFactorCVService.EnsureSubTypeOfUseExistsAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 999, YearRangeCVId = 1, Factor = 1.0m };

        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.SubTypeOfUseId), "Sub type of use with ID 999 not found.", OperationType.Create));

        await Assert.ThrowsAsync<ValidationException>(() => controller.Create(createDto, CancellationToken.None));
    }

    [Fact]
    public async Task Create_WithSubTypeOfUseNotBelongingToTypeOfUse_ThrowsValidationException()
    {
        // Business rule enforced by AssetUseFactorCVService.EnsureSubTypeOfUseExistsAsync's
        // TypeOfUseId/SubTypeOfUseId relationship check
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 2, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m };

        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.SubTypeOfUseId), "Sub type of use with ID 1 not found for type of use 2.", OperationType.Create));
    ...
    {
        // Business rule enforced by AssetUseFactorCVService.EnsureYearRangeExistsAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 999, Factor = 1.0m };
    ...
    [Fact]
    public async Task Create_WithDuplicateCombination_ThrowsValidationException()
    {
        // Business rule enforced by AssetUseFactorCVService.ValidateForCreateAsync
        var controller = Create(out var service);
        var createDto = new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m };

        service.Setup(s => s.CreateAsync(createDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(createDto.YearRangeCVId), "UseFactorCV_Combination_Duplicate", OperationType.Create));
    ...
    public async Task Create_WhenNonValidationDuplicateConstraintErrorOccurs_ReturnsConflict()
    {
        ...
        var response = Assert.IsType<ApiResponse<AssetUseFactorCVMasterDto>>(conflictResult.Value);
    ...
    {
        var controller = Create(out var service);
        var updateDto = new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.95m, IsActive = true, UpdatedBy = 1 };
        var updatedDto = new AssetUseFactorCVMasterDto { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.95m, IsActive = true };
    ...
    {
        var controller = Create(out var service);
        var updateDto = new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m };
        service.Setup(s => s.UpdateAsync(999, updateDto, It.IsAny<CancellationToken>())).ReturnsAsync((AssetUseFactorCVMasterDto?)null);
    ...
    [Fact]
    public async Task Update_WithNonExistentTypeOfUseId_ThrowsValidationException()
    {
        var controller = Create(out var service);
        var updateDto = new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 999, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m };

        service.Setup(s => s.UpdateAsync(1, updateDto, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ValidationException(nameof(updateDto.TypeOfUseId), "Type of use with ID 999 not found.", OperationType.Update));
    ...
    {
        // Business rule enforced by AssetUseFactorCVService.ValidateForDeactivationAsync
        var controller = Create(out var service);
        var updateDto = new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m, IsActive = false };
    ...
    {
        // Business rule enforced by AssetUseFactorCVService.ValidateForDeleteAsync
        var controller = Create(out var service);
    ...
    public async Task BulkCreate_WithValidItems_ReturnsOkWithSuccessCount()
    {
        ...
        var items = new[]
        {
            new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m },
            new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 2, YearRangeCVId = 1, Factor = 0.9m }
        };
        var createdItems = new List<AssetUseFactorCVMasterDto>
        {
            new() { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m },
            new() { Id = 2, TypeOfUseId = 1, SubTypeOfUseId = 2, YearRangeCVId = 1, Factor = 0.9m }
        };
        var bulkResult = new BulkResult<AssetUseFactorCVMasterDto>(2, 0, createdItems);
        ...
        var okResult = Assert.IsType<OkObjectResult>(result);
        var response = Assert.IsType<ApiResponse<BulkResult<AssetUseFactorCVMasterDto>>>(okResult.Value);
        Assert.True(response.Success);
    ...
    public async Task BulkCreate_WithNullArray_ReturnsBadRequest()
    {
        ...
        var response = Assert.IsType<ApiResponse<BulkResult<AssetUseFactorCVMasterDto>>>(badRequest.Value);
        Assert.False(response.Success);
    }
    ...
    {
        var controller = Create(out _);
        var items = Array.Empty<CreateAssetUseFactorCVMasterDto>();
    ...
    public async Task BulkCreate_WithPartialFailures_ReturnsOkWithMixedResults()
    {
        ...
        {
            new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m },
            new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 999, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.9m }
        };
        var createdItems = new List<AssetUseFactorCVMasterDto> { new() { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m } };
        var bulkResult = new BulkResult<AssetUseFactorCVMasterDto>(1, 1, createdItems, new List<string> { "Item 1: TypeOfUseId not found" });
    ...
    {
        var controller = Create(out var service);
        var items = new[] { new CreateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 1.0m } };
        ...
        var conflictResult = Assert.IsType<ConflictObjectResult>(result);
        var response = Assert.IsType<ApiResponse<BulkResult<AssetUseFactorCVMasterDto>>>(conflictResult.Value);
        Assert.False(response.Success);
    }
    ...
    public async Task BulkUpdate_WithValidItems_ReturnsOkWithSuccessCount()
    {
        ...
        var items = new[]
        {
            new BulkUpdateItem<int, UpdateAssetUseFactorCVMasterDto>(1, new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.95m }),
            new BulkUpdateItem<int, UpdateAssetUseFactorCVMasterDto>(2, new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 2, YearRangeCVId = 1, Factor = 0.85m })
        };
        var updatedItems = new List<AssetUseFactorCVMasterDto>
        {
            new() { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.95m },
            new() { Id = 2, TypeOfUseId = 1, SubTypeOfUseId = 2, YearRangeCVId = 1, Factor = 0.85m }
        };
        var bulkResult = new BulkResult<AssetUseFactorCVMasterDto>(2, 0, updatedItems);
    ...
    public async Task BulkUpdate_WithNonExistingIds_ReturnsPartialFailure()
    {
        ...
        {
            new BulkUpdateItem<int, UpdateAssetUseFactorCVMasterDto>(1, new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.95m }),
            new BulkUpdateItem<int, UpdateAssetUseFactorCVMasterDto>(999, new UpdateAssetUseFactorCVMasterDto { TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.85m })
        };
        var updatedItems = new List<AssetUseFactorCVMasterDto> { new() { Id = 1, TypeOfUseId = 1, SubTypeOfUseId = 1, YearRangeCVId = 1, Factor = 0.95m } };
        var bulkResult = new BulkResult<AssetUseFactorCVMasterDto>(1, 1, updatedItems, new List<string> { "Record with Id '999' not found." });

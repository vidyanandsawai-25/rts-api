...
{
    public class UpdateRuleEngineDtoJsonConverter : JsonConverter<UpdateRuleEngineDto>
    {
        public override UpdateRuleEngineDto? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            using var jsonDoc = JsonDocument.ParseValue(ref reader);
            var root = jsonDoc.RootElement;

            var dto = new UpdateRuleEngineDto();
            ...
            // Map IsActive (from UpdateBaseDtos)
            ...
            // Map UpdatedBy (from UpdateBaseDtos)
            if (root.TryGetProperty("updatedBy", out var ubProp) || root.TryGetProperty("UpdatedBy", out ubProp))
            {
                if (ubProp.ValueKind == JsonValueKind.Number && ubProp.TryGetInt32(out var ubVal))
                    dto.UpdatedBy = ubVal;
            }
        ...
        public override void Write(Utf8JsonWriter writer, UpdateRuleEngineDto value, JsonSerializerOptions options)
        {
            ...
            if (value.UpdatedBy.HasValue)
                writer.WriteNumber("updatedBy", value.UpdatedBy.Value);
            else

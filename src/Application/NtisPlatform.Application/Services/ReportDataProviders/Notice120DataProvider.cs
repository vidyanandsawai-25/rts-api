...
namespace NtisPlatform.Application.Services.ReportDataProviders
{
    ...
    public class Notice120DataProvider : IPagedReportDataProvider
    {
        ...
        private async Task<List<object>> BuildMainRowAsync(int propertyId, int userId, CancellationToken ct)
        {
            ...
            // 1c. Society details (LEFT JOIN on PropertyId)
            ...
            // 1d. Type-of-use (LEFT JOIN on PM.PropertyTypeId = TUM.Id)
            ...
            // 1e. PropertyMastOld (LEFT JOIN on PM.PropertyMastOldId = PMO.Id)
            ...
            // 1f. PropertyTypeMaster (LEFT JOIN on PM.PropertyTypeId = PTM.Id)
            ...
            // 1g. ULB Master — SELECT from [CORE].[UlbMaster] (first/only row)
            ...
            // 1h. User: select from [CORE].[UserMaster] where Id = @userId

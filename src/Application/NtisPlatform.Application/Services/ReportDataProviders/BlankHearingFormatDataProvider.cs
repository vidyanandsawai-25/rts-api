    ...
    private async Task<(List<object> Rows, bool HasMore)> BuildPageAsync(Dictionary<string, string> parameters, int skip, int take, CancellationToken ct)
    {
        ...
        var propQuery =
            ...
            from sdm in sdmj.DefaultIfEmpty()
                ...
                //join opm in _PropertyMastOldRepository.GetQueryable() on pm.Id equals opm.Id into opmj
            join opm in _PropertyMastOldRepository.GetQueryable() on pm.PropertyMastOldId equals opm.Id into opmj
            from opm in opmj.DefaultIfEmpty()

                // new join: PropertyMastOld -> aggregated TransMastOld
            join tmoa in transOldAggQuery
                on opm.Id equals tmoa.PropertyMastOldId into tmoaj

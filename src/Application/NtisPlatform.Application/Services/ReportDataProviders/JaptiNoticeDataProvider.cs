...
namespace NtisPlatform.Application.Services.ReportDataProviders
{
    ...
    public class JaptiNoticeDataProvider : IPagedReportDataProvider
    {
            ...
            Dictionary<string, string> parameters, int skip, int take, CancellationToken ct)
        {
            ...
            // --- Build one output row per propertyId (preserves the requested order) ---
            var allRows = new List<Dictionary<string, object?>>();

            foreach (var pid in propertyIds)
            {
                var property = properties.FirstOrDefault(p => p.Id == pid);
                wardMap.TryGetValue(property?.WardId ?? 0, out var wardNo);

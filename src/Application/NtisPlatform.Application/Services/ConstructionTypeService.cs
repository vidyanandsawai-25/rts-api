...
public class ConstructionTypeService : BaseCommonCrudService<ConstructionTypeEntity, ConstructionTypeDto, CreateConstructionTypeDto, UpdateConstructionTypeDto, ConstructionTypeQueryParameters, int>, IConstructionTypeService
{
    ...
    /// <summary>
    /// Validates deactivation (IsActive change from true to false) for ConstructionTypeEntity.
    /// Uses centralized IReferenceValidationService to check references in related tables.
    /// </summary>
    protected override async Task<ValidationResult> ValidateForDeactivationAsync(
        int id,
        ConstructionTypeEntity currentEntity,
        ConstructionTypeEntity updatedEntity,
        CancellationToken cancellationToken = default)
    {
        if (currentEntity.IsActive && !updatedEntity.IsActive)
        {
            return await _referenceValidator.ValidateReferencesAsync<ConstructionTypeEntity>(id, cancellationToken);
        }
